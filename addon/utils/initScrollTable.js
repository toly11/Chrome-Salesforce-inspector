...
export function initScrollTable(scroller) {
    ...
    const INITIAL_ROW_HEIGHT = 15; // The initial estimated height of a row before it is rendered
    const INITIAL_COL_WIDTH = 50; // The initial estimated width of a column before it is rendered
    const BUFFER_HEIGHT = 500; // The number of pixels to render above and below the current viewport
    const BUFFER_WIDTH = 500; // The number of pixels to render to the left and right of the current viewport
    const HEADER_ROWS = 1; // The number of header rows
    const HEADER_COLS = 0; // The number of header columns
    ...
    function dataChange(newData) {
        ...
        if (data == null || data.rowVisibilities.length == 0 || data.colVisibilities.length == 0) {
            ...
            for (let r = rowCount; r < newRowCount; r++) {
                rowHeights[r] = INITIAL_ROW_HEIGHT;
                rowVisible[r] = 0;
            }
    ...
    function renderData({ force }) {
        ...
        while (firstRowTop < scrollTop - BUFFER_HEIGHT && firstRowIdx < rowCount - 1) {
            firstRowTop += rowVisible[firstRowIdx] * rowHeights[firstRowIdx];
            firstRowIdx++;
        }
        while (firstRowTop > scrollTop - BUFFER_HEIGHT && firstRowIdx > 0) {
            firstRowIdx--;
            firstRowTop -= rowVisible[firstRowIdx] * rowHeights[firstRowIdx];
        }
        while (firstColLeft < scrollLeft - BUFFER_WIDTH && firstColIdx < colCount - 1) {
            firstColLeft += colVisible[firstColIdx] * colWidths[firstColIdx];
            firstColIdx++;
        }
        while (firstColLeft > scrollLeft - BUFFER_WIDTH && firstColIdx > 0) {
            firstColIdx--;
            firstColLeft -= colVisible[firstColIdx] * colWidths[firstColIdx];
        }
        ...
        while (lastRowTop < scrollTop + offsetHeight + BUFFER_HEIGHT && lastRowIdx < rowCount) {
            lastRowTop += rowVisible[lastRowIdx] * rowHeights[lastRowIdx];
            lastRowIdx++;
        }
        lastColIdx = firstColIdx;
        lastColLeft = firstColLeft;
        while (lastColLeft < scrollLeft + offsetWidth + BUFFER_WIDTH && lastColIdx < colCount) {
        ...
        for (let r = firstRowIdx; r < lastRowIdx; r++) {
            ...
            for (let c = firstColIdx; c < lastColIdx; c++) {
                ...
                if (r < HEADER_ROWS || c < HEADER_COLS) {
                    td.className += " header";
                }
    ...
    function popLink(recordInfo, label) {
      ...
      a.addEventListener("click", e => {
        ...
        for (let objectType of objectTypes) {
          ...
          aShow.href = "/modules/inspect/inspect.html?" + args;
